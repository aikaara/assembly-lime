...
function getGeminiCliClientId(): string {
	const envVal = process.env.GOOGLE_GEMINI_CLI_CLIENT_ID;
	if (envVal) return envVal;
	const p1 = "NjgxMjU1ODA5Mzk1LW9vOGZ0Mm9w";
	const p2 = "cmRybnA5ZTNhcWY2YXYzaG1kaWIx";
	const p3 = "MzVqLmFwcHMuZ29vZ2xldXNlcmNv";
	const p4 = "bnRlbnQuY29t";
	return atob(p1 + p2 + p3 + p4);
}
function getGeminiCliClientSecret(): string {
	const envVal = process.env.GOOGLE_GEMINI_CLI_CLIENT_SECRET;
	if (envVal) return envVal;
	const p1 = "R09DU1BYLTR1SGdN";
	const p2 = "UG0tMW83U2stZ2VW";
	const p3 = "NkN1NWNsWEZzeGw=";
	return atob(p1 + p2 + p3);
}
const CLIENT_ID = getGeminiCliClientId();
const CLIENT_SECRET = getGeminiCliClientSecret();
