...
function getAntigravityClientId(): string {
	const envVal = process.env.GOOGLE_ANTIGRAVITY_CLIENT_ID;
	if (envVal) return envVal;
	const p1 = "MTA3MTAwNjA2MDU5MS10bWhz";
	const p2 = "c2luMmgyMWxjcmUyMzV2dG9s";
	const p3 = "b2poNGc0MDNlcC5hcHBzLmdv";
	const p4 = "b2dsZXVzZXJjb250ZW50LmNvbQ==";
	return atob(p1 + p2 + p3 + p4);
}
function getAntigravityClientSecret(): string {
	const envVal = process.env.GOOGLE_ANTIGRAVITY_CLIENT_SECRET;
	if (envVal) return envVal;
	const p1 = "R09DU1BYLUs1OEZX";
	const p2 = "UjQ4NkxkTEoxbUxC";
	const p3 = "OHNYQzR6NnFEQWY=";
	return atob(p1 + p2 + p3);
}
const CLIENT_ID = getAntigravityClientId();
const CLIENT_SECRET = getAntigravityClientSecret();
